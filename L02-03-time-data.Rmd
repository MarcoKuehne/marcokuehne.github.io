...
The time system in which 1 hour is divided into 60 minutes, and each minute is further divided into 60 seconds, is called the **sexagesimal** or **base-60** system. This system has been widely used in measuring time and angles (see Section \@ref(geo-data)).
...
### Measuring Dates {#measuring-dates}
...
There are 24 time zones, each representing 15 degrees of longitude (see Section \@ref(geo-data)). Time zones are centred around the Prime Meridian (0 degrees longitude).
...
```{r tim-time-zones, echo=TRUE, warning=FALSE, message=FALSE}
library(lubridate)
...
### Time Management in R {#time-management-in-r}
...
```{r tim-time-formats, echo=FALSE, eval=FALSE}
...
```{r tim-time-formats-2, echo=TRUE}
...
### Coffee Spending {#coffee-spending}
...
```{r tim-coffee-spending, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r tim-coffee-spending-2, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r tim-coffee-spending-3, echo=TRUE, warning=FALSE, message=FALSE}
# First and Last Date
range(Coffee$Date)
...
```{r tim-spending-time-day, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r tim-spending-time-day-2, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}
library(chron)
...
```{r tim-run-chart, echo=TRUE, warning=FALSE, message=FALSE, fig.align='center', fig.cap="Every purchase in the coffee app between November 2022 and June 2023, in euros. Flat stretches are days without spending."}
...
```{r tim-run-chart-grouped, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.cap="The same spending coloured by purchase type. Mensa appears twice, once misspelled Mensq, and drinks tower over coffee."}
...
### Run Chart Grouped Cleaned {#run-chart-grouped-cleaned}

Filter other beverages and correct the `mensq` typo.

```{r tim-run-chart-grouped-2, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.cap="After dropping drinks and fixing the Mensq typo, horizontal lines mark the mean spend for Mensa and coffee."}
Coffee_Clean <- Coffee %>% 

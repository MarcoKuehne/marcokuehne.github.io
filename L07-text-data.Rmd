...
This book has argued from Chapter \@ref(tabular-data) onwards that rectangular data — one row per observation, one column per variable — is the workhorse structure of empirical research. Text seems to fit fine. A review is a value; put it in a cell.
...
The course questionnaire from Chapter \@ref(synthetic-data) had one column we ignored there. Alongside age, degree and semesters, every student wrote a free-text answer to a single question: *what do you expect from this course?*
...
Note what this is. The same file gave us a rectangular data set in Chapter \@ref(synthetic-data) and a corpus here. Text is often not a separate kind of study — it is the column everybody drops.
...
::: {.analysis-note}
This is the same kind of moment as in Chapter \@ref(geo-data), where coordinates, projections and shapefiles arrived because points on a curved earth do not fit in two ordinary numeric columns.

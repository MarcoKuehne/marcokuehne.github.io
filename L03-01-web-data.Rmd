...
**Make the table talk.** Two derived columns turn a list of digits into a story: the share of international students and the change against the same semester one year earlier.

```{r viadrina-table}
viadrina_display <- viadrina %>%
  # Compare summer with summer and winter with winter, never across terms.
  group_by(term) %>%
  mutate(change = students / lag(students) - 1) %>%
  ungroup() %>%
  # Newest semester on top.
  arrange(desc(year), desc(term)) %>%
  transmute(
    Semester = semester,
    Students = students,
    `vs. year before` = change,
    German = german,
    International = international,
    `International share` = international / students,
    Women = women,
    Men = men,
    `First semester` = freshers
  )

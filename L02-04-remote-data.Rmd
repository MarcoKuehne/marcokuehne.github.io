...
::: {.analysis-warning}
**Watch out.** `collect()` is the moment where the data actually moves. Put it at the **end** of a pipeline, after filtering and summarising — never at the beginning. `tbl(con, "sales") %>% collect() %>% filter(...)` asks the database for all forty million rows and then throws most of them away, which is the single most common mistake with `dbplyr`.
:::
...
```{r api-plot, fig.align='center', fig.width=7, fig.height=4.5, message=FALSE, warning=FALSE, fig.cap="Marshall Islands GDP fetched from the World Bank API: the whole economy has never matched one painting's auction price."}
gdp %>%
...
### The same idea twice {#the-same-idea-twice}

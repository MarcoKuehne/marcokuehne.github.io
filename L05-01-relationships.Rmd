...
### Storks Deliver Babies {#storks-deliver-babies}

```{r rel-storks-deliver-babies, echo=FALSE, out.width="100%", fig.cap="Stork bringing baby - Colmar, Alsace.", fig.align='center'}
...
```{r rel-storks-deliver-babies-2, echo=FALSE, message=FALSE, warning=FALSE}
library(readxl)
...
# Tabular Data
datatable(caption = "The 2005 stork census for 28 countries: stork pairs, area, population, urban population and fertility rate.", storks, options = list(iDisplayLength = 5))
```

`Storks` is the number of pairs of storks in that country. The `Area` is in square kilometres. `Population` is the total population in million whereas `UrbanPop` is the population living in urban areas. `Fertility` refers to the total fertility rate, that is the average number of children that would be born to a female over their lifetime. How many storks have been observed in Ukraine in 2005? Answer: `r fitb(30000)`. Which country has more storks? `r mcq(c("Germany", "Greece", answer = "Hungary"))`
...
```{r rel-variance, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Stork pairs per country across Eurasia; darker countries hold more pairs, and that spread is what variance measures."}
...
To calculate the variance, we subtract each data point $x_i$ from the mean $\bar{x}$. Then square those deviations and add them up. Finally, there is a scaling factor, we divide by the number of observations minus 1. 
...
$$\text{sd}(x) = \sqrt{\text{var}(x)} = \sqrt{ \frac{\sum (x_i - \bar{x} )^2}{n-1} } $$
In R, the standard deviation is calculated by `sd()` like `sd(storks$Storks)` which yields `r format(round(sd(storks$Storks),0), scientific=FALSE)`. The mean of storks `mean(storks$Storks)` is approximately `r round(mean(storks$Storks),0)`. The one-dimensional variable `Storks` is visualised as points on the line.  

```{r rel-standard-deviation, echo=FALSE, eval=TRUE, out.width='80%', fig.align="center", fig.cap="Each country's stork count as a point on one line; the arrows mark one standard deviation either side of the mean."}
...
```{r rel-standard-deviation-2, echo=FALSE, eval=TRUE, out.width='80%', fig.align="center", fig.cap="Storks against population with each variable's mean in red; most countries fall in the lower-left or upper-right quadrant."}
...
$$\text{cov}(x,y) = \frac{1}{n-1} \sum\limits (x_i - \bar{x})(y_i - \bar{y})$$ 
...
In R, the covariance is calculated by `cov(storks$Storks, storks$Population)` and yields `r format(round(cov(storks$Storks, storks$Population),0), scientific=FALSE)`. The positive covariance confirms what we saw in the first scatterplot, the positive association between storks and population. Let's try to visualise the calculation procedure of the covariance. 

```{r rel-covariance, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, out.width='80%', fig.align="center", fig.cap="Each rectangle spans one country's deviations from both means; covariance is the sum of these signed areas."}
...
Covariance qualifies a relationship as positive or negative, i.e. the direction of the relationship. Covariance is expressed in units that vary with the data. Because the data are not standardised, you cannot use the covariance statistic to assess the strength of a linear relationship (a covariance of 117.5 can be very low for one relationship and 0.23 very high for another relationship). 

We need a measure independent of units in a fixed range, the correlation coefficient. 

#### Correlation 

To assess the strength of a relationship between two variables a correlation coefficient is commonly used. It brings variation to a standardised scale between -1 to +1.
...
```{r rel-correlation-2, echo=TRUE, message=FALSE, warning=FALSE}
# Advanced function 
...
The correlation coefficient is the same value no matter the order of the variables. It is symmetric, i.e. the correlation between variables $x$ and $y$ is the same as the correlation between $y$ and $x$. However, the regression coefficient is a bit picky. It does care about the order of things. If you swap the roles of the dependent ($y$) and independent variables ($x$), the regression coefficient changes. So, it's like having a favourite direction – it matters which way you're looking when interpreting the relationship.

```{r rel-early-glimpse-into, echo=FALSE}
# Provided data
...
df_wide %>% 
  ...
  datasummary_df(fmt = fmt_significant(digits = 3),
                 title = "Five observations of x and y with their variances, covariance, correlation and regression slope; the last two differ.")
```

Notably, if the standard deviations ($\sigma$) of the two variables are equal, the correlation coefficient and the simple regression coefficient coincide. We can achieve this when variables are on the same scale. The most common way of achieving this is through standardisation.  

**Standardisation** is a crucial process in statistics that harmonizes variables by centring them around a mean of 0 and scaling them to have a standard deviation of 1. This transformation ensures that variables operate on a uniform scale, facilitating meaningful comparisons. The standardisation formula is expressed as:

$$ z = \frac{x - \bar{x}}{\sigma} $$

By subtracting each data point from the mean and dividing by the standard deviation, we create a standardised variable that simplifies the interpretation and comparison of different datasets. The same calculation based on scaled data: 

```{r rel-early-glimpse-into-2, echo=FALSE}
...
df_scaled_wide %>% 
  #mutate(across(c(everything(), any_of(cols_2)),round, 0 )) %>% 
  datasummary_df(fmt = fmt_significant(digits = 3),
                 title = "The same five observations after standardisation: both variances are one, so correlation and regression slope now coincide.")
```


### Visualisations {#visualizations}

```{r rel-visualisations, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
storks <- storks %>% 
...
```{r rel-storks-population-scatterplot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Stork pairs against population for 28 countries; the fitted line slopes upwards, a positive but hardly causal association."}
library(ggpmisc)
...
```{r rel-storks-population-barplot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Mean population within each quartile of stork numbers; the labels give the range of stork pairs behind each bar."}
storks %>%
...
Logarithmic transformations are particularly useful for variables that exhibit skewed distributions, large ranges, or multiplicative relationships. Examples include income, where logarithmic transformation can help normalise the data, making it more symmetric. Population growth rates are often better visualised on a log scale due to their multiplicative nature. Stock prices, influenced by exponential growth or decay, benefit from log scales to highlight percentage changes and identify trends. 

This compression can be particularly useful when you have extreme values that would otherwise make the visualisation challenging.

```{r rel-storks-area, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="Storks against country area on a linear (left) and logarithmic (right) scale; the log scale tames the extreme countries."}
library(patchwork)
...
```{r rel-storks-urban-population, message=FALSE, warning=FALSE, echo=FALSE, fig.cap="Stork pairs against the urban share of the population, one of the common causes behind the stork-baby correlation."}
storks %>% 
  mutate(UrbanShare = UrbanPop/Population) %>% 
...
### Spurious Relationships {#spurious-relationships}

```{r rel-spurious-relationships, echo=FALSE, out.width="60%", fig.cap="The first known pair in Finland (2015), representing a northward expansion compared to the species' historical breeding range.", fig.align='center'}
...
```{r rel-spurious-relationships-2, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
library(RSelenium)

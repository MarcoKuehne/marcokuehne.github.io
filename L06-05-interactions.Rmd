...
```{r int-data-sample, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
### Throwback Parallel Slopes {#throwback-parallel-slopes}
...
```{r int-throwback-parallel-slopes, message=FALSE, warning=FALSE, echo=FALSE, eval=TRUE, fig.cap="Income against age by gender, forced into parallel slopes: one common age effect, two different starting levels."}
...
```{r int-throwback-parallel-slopes-2, message=FALSE, warning=FALSE, echo=FALSE, eval=TRUE}
...
### Regression with Moderators {#regression-with-moderators}
...
```{r int-dummy-dummy-2, fig.cap="Predicted income by having children, separately for women and men; the child effect changes sign between the two."}
...
```{r int-categorical-dummy-2, fig.cap="Predicted income by industry for women and men; the gender gap is far wider in some industries than others."}
...
```{r int-continuous-dummy-2, warning=FALSE, fig.cap="Income over age fitted separately for women and men; the slopes differ, so the gap widens with age."}
...
```{r int-continuous-dummy-3, fig.cap="Dropping the gender main effect ties both lines to one intercept, leaving only the slopes free to differ."}
...
```{r int-continuous-continuous-2, fig.cap="Income over age at three levels of life satisfaction: the mean and one standard deviation either side."}
...
:::: {.dedicated}
::: {.titeldedicated}
<h2>Truly Dedicated: why not just split the sample?</h2>
:::
...
[^int-split]: Le, A., *Interaction Term vs Split Sample*, 2016, <https://anhqle.github.io/interaction-term/>. The same point is worked through in the Cross Validated thread *Interaction effect in a multiple regression vs. split sample*, <https://stats.stackexchange.com/questions/155895/>.

[^int-gelman]: Gelman, A., & Stern, H. (2006). The Difference Between "Significant" and "Not Significant" Is Not Itself Statistically Significant. *The American Statistician* 60(4), 328-331, <https://doi.org/10.1198/000313006X152649>.

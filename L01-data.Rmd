...
::: {.challenge}
Look up the codebook of *pequiv* dataset from SOEP <https://www.diw.de/documents/publikationen/73/diw_01.c.704530.de/diw_ssp0772.pdf>
:::

<!-- Let's test the old verison of the `webex` package. Install an old version of R package via  `install.packages("https://cran.r-project.org/src/contrib/Archive/webex/webex_0.9.2.tar.gz", repos=NULL, type="source")`. Better use the new version `webexercises`. -->

* What is the variable `P11101`? `r fitb(c("Happiness", "happiness", ignore_case = TRUE))`  (Hint, use a single word) 

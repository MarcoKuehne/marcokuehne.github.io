...
### Why the tidyverse? {#why-the-tidyverse -}
...
### R is a calculator {#r-is-a-calculator -}
...
### R is more than a calculator {#r-is-more-than-a-calculator -}
...
### Objects: giving things a name {#objects-giving-things-a-name -}
...
`na.rm = TRUE` is a decision, not a formality. Section \@ref(missing-data) returns to what dropping those cases quietly assumes.
...
### Functions and their arguments {#functions-and-their-arguments -}
...
### Plots in base R {#plots-in-base-r -}
...
```{r base-plot-lines, fig.cap="Base graphics built in layers: plot() opens the frame with x squared in red, lines() adds twice that in blue."}
x  <- 1:10
y1 <- x * x
y2 <- 2 * y1
...
## Intro to the Tidyverse {#intro-to-tidyverse -}
...
### The verbs of dplyr {#the-verbs-of-dplyr -}
...
### Graphs with ggplot2 {#graphs-with-ggplot2 -}
...
## RStudio and projects {#rstudio-projects -}
...
- **Posit** is the company that makes it. It was called RStudio, Inc. until [November 2022](https://posit.co/blog/rstudio-is-now-posit/), when it renamed itself to signal that it now builds for Python as well as R: *"not about pivoting from R to Python … about broadening and embracing the Python community as well as the R community"*. The IDE deliberately kept its old name.
...
### The four panes {#the-four-panes -}

```{r rstudio-panes, echo=FALSE, out.width="100%", fig.align="center", fig.cap="The RStudio window. Schematic, because the real one moves its furniture with every release."}
knitr::include_graphics("images/rstudio-panes.svg")
```
...
### The project is the unit of work {#the-project-is-the-unit-of-work -}
...
:::: {.challenge}
::: {.titelchallenge}
<h2>Your Turn: The thirty-second diagnosis</h2>
:::
...
### A folder that scales {#a-folder-that-scales -}
...
### Restart R, often {#restart-r-often -}
...
## Version control, briefly {#version-control -}
...
### The thing you already know from Wikipedia {#the-thing-you-already-know-from-wikipedia -}
...
<h2>Amazing Fact</h2>
:::
That history tab is also a research tool, and it comes back in Section \@ref(web-data), where we scrape a Wikipedia table of the most expensive paintings ever sold.
...
### GitHub is also a library {#github-is-also-a-library -}
...
<center><https://github.com/jobreu/youtube-workshop-gesis-2022></center>
...
### Your own history {#your-own-history -}
...
And it closes the circle this chapter opened. [Why code?](#why-code) argued that a result is only as good as the trail that leads to it. Version control extends that trail backwards in time: not just *how* this number was produced, but *when the recipe changed, and why somebody thought that was a good idea*.

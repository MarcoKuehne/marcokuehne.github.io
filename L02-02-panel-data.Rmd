...
```{r pan-decline-germany, message=FALSE, warning=FALSE, echo=FALSE, fig.align='center', fig.cap='Unemployment rate in Germany'}
...
![](images/lifeaftercollege.png "Life After College"){width=350px}
</details>
</center>
...
```{r pan-data-inspection-2, warning=FALSE, message=FALSE, echo=FALSE, eval=FALSE}
...
```{r pan-data-inspection-3, message=FALSE, warning=FALSE, echo=FALSE, eval=TRUE}

remove_all_labels(soep) %>% 
  select(2:15) %>% 
  datasummary_skim(title = "One row per SOEP practice variable, with unique values, missing share, mean, spread, range and a small histogram.") #|>
...
```{r pan-data-inspection-4, warning=FALSE, message=FALSE, echo=TRUE, eval=TRUE}
...
```{r pan-data-inspection-5, warning=FALSE, message=FALSE, echo=TRUE, eval=TRUE}
...
```{r pan-data-inspection-6, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
...
```{r pan-data-inspection-7, warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE}
soep %>% count(id) %>% arrange(n)
```
</details>
...
```{r pan-data-inspection-8, warning=FALSE, message=FALSE, echo=TRUE, eval=TRUE}
table(soep$syear)
```
...
```{r pan-data-inspection-9, warning=FALSE, message=FALSE, echo=TRUE, eval=TRUE}
...
```{r pan-data-inspection-10, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
...
```{r pan-data-inspection-11, warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE}
attributes(soep$erwerb)$labels
```
...
We summarise categories with a combination of `mutate()` and `case_when()`. For each value in `erwerb` conduct a logical comparison via `==` and assign a new value. In this case it combines *Employed part-time* with *Irregular employment or in marginal*.

```{r pan-data-preparation}
soep <- soep %>% 
...
```{r pan-data-preparation-3, echo=TRUE, eval=FALSE}
soep %>% 
...
```{r pan-data-preparation-4, echo=FALSE, eval=TRUE}
soep %>% 
  group_by(syear, erwerb) %>%
  summarise(n = n())
```
...
Having done this, step back to focus on years again and use different n-values per year to figure out the unemployment. Relate `unemployed` to those working either `fulltime` or `parttime`. 

```{r pan-data-preparation-5, echo=TRUE, eval=FALSE}
soep %>% 
  group_by(syear, erwerb) %>%
  summarise(n = n()) %>% 
  group_by(syear) %>% 
...
```{r pan-data-preparation-6, echo=FALSE, eval=TRUE}
soep %>% 
  group_by(syear, erwerb) %>%
  summarise(n = n()) %>% 
  group_by(syear) %>% 
...
#### Data Visualisation {#pan-data-visualisation}
...
```{r pan-unemployment-status, fig.align='center', class.source='fold-hide', fig.cap="Unemployment rate in the SOEP practice data falls from 28 to 23 per cent, far above the official German figure."}
soep %>% 
  group_by(syear, erwerb) %>%
  summarise(n = n()) %>% 
  group_by(syear) %>% 
...
```{r pan-obs-per-individual, fig.align='center', fig.cap="How many of the five SOEP waves each person appears in. Over a third are observed every year."}
soep %>% 
  group_by(id) %>% 
  count() %>% 
...
These scientific datasets can often be analysed for research and student theses free of charge.  

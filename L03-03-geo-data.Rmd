...
Analysing and visualising geospatial data is essential in various fields, including geography, environmental sciences, urban planning, epidemiology, and transportation not to mention social science.
...
```{r geo-where-are-you, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Two points a few metres apart in Frankfurt (Oder): the Viadrina main building and the best coffee in town."}
...
Berlin, the capital of Germany, is located:
...
$$\text{Decimal degree} = \text{Degree} + \frac{\text{Minute}}{60} + \frac{\text{Second}}{3600}$$

- Latitude: 52.5200° N
- Longitude: 13.4050° E
   
```{r geo-angles-degrees, echo=FALSE, fig.cap="The same outline of Germany twice, with Berlin labelled in degrees, minutes and seconds on the left and decimal degrees on the right."}
...
In general, coordinates with six decimal places (0.000001 degrees) can provide location accuracy to approximately within a few centimetres. Each additional decimal place adds further precision, narrowing down the location to smaller units of measurement.

The entrance of Viadrina main building and the best coffee in town are about 40m away. 

```{r geo-angles-degrees-2, echo=FALSE, fig.cap="The same two addresses on a zoomable map, joined by a red line roughly 40 metres long."}
...
```{r geo-angles-degrees-3, echo=FALSE}

# Display the data frame as a styled HTML table
kable(caption = "The two Frankfurt (Oder) addresses as plain numbers: latitude differs in the fourth decimal, longitude in the third.", FFO.df, format = "html") %>%
...
```{r geo-coordinate-reference-system, echo=TRUE}
...
```{r geo-coordinate-reference-system-2, echo=FALSE, fig.cap="Germany in WGS 84 on the left and in a CRS made for Iceland on the right; the same country, differently bent."}
# Extract Germany's geometry
germany <- ne_countries(country = "germany", returnclass = "sf")
...
# Display the data frame as a styled HTML table
kable(caption = "The same pair of coordinates measured three ways; only the Vincenty ellipsoid allows for the Earth being flattened at the poles.", distance_data, format = "html") %>%
  kable_styling("striped", full_width = FALSE) %>% 
...
Polygon data and geographical coordinates work together to represent and visualise spatial areas on a map. In a geographical context, polygons are commonly used to define the boundaries of regions, countries, cities, or any other geographic entities.

```{r geo-points-polygons, fig.cap="A rectangle running from 5 to 15 degrees east and 47 to 55 north, standing in for Germany."}
...
Polygon data for countries typically comes from geospatial databases, GIS (Geographic Information System) data, or **shapefiles** maintained by government agencies, international organisations, or open-source initiatives.
...
```{r geo-points-polygons-2, animation.hook="gifski", echo=FALSE, fig.align='center', fig.width=4, fig.height=5.5, warning=FALSE, message=FALSE, fig.cap="Germany's outline redrawn from ten dots up to a hundred; the country only becomes recognisable towards the end."}
library(jsonlite)
...
```{r geo-shapefiles, echo=FALSE, warning=FALSE, message=FALSE}
library(htmltools)
...
```{r geo-google-takeout, fig.cap="One month of Google location history, January 2023, with every recorded place visit as a labelled marker."}
# Load JSON data
...
Blood donation is a vital contribution to healthcare, saving countless lives every day. In Germany, the German Red Cross (Deutsches Rotes Kreuz) plays a crucial role in organising and facilitating blood donation events across the country. You can find information about blood donation venues on their official website:

<https://www.blutspenden.de/blutspendedienste/>

The code fetches, processes, and extracts information about blood donation locations in Germany from a website, organising the data into a tidy tibble for further analysis and mapping.

```{r geo-blood-donation, eval=FALSE}
library('rvest')
library('stringi')
...
```{r blood-donation-data, message=FALSE, warning=FALSE}
...
blood %>%
  ...
  DT::datatable(caption = "The 2023 snapshot of 248 German blood donation venues, with town, postcode and coordinates for each.", 
    rownames = FALSE,
    options = list(
      dom = "tip",
      scrollX = TRUE,
...
```{r geo-blood-donation-2, fig.cap="All 248 donation venues as red dots on an outline of Germany; they crowd around the larger cities."}
library("maps")

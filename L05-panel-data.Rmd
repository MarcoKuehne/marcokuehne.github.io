...
<!-- -   Data Skills: -->
<!--       +   How `set.seed()` defeats randomness.   -->
<!--       +   Find and use *functions* from other people.  -->
<!--       +   Tidyverse e.g. `group_by()`, `sample_n()`, `summarize()`, `count()`, `arrange()`. -->
...
[How Low Can Unemployment Really Go? Economists Have No Idea](https://www.nytimes.com/2018/02/28/upshot/how-low-can-unemployment-really-go-economists-have-no-idea.html)
:::
...
This more comprehensive practice data contains socio-economic information on children, education, job, health, satisfaction and income. It contains `r ncol(soep)` variables and `r nrow(soep)` observations. 
```{r message=FALSE, warning=FALSE, echo=F, eval=T}
...
The documentation (click the DOI <https://doi.org/10.5684/soep.practice.v36>) tells that there are 6.355 people in the data. Every individual is likely observed multiple times (remember: panel data). Pipe `soep` into `count()` of personal `id`. This tibble output already contains the number of rows. To literally access the value, ask `nrow()`.
...
Remember that the observation period is between `r min(soep$syear)` and `r max(soep$syear)`, i.e. the minimum number of observations per individual is 1 year, the maximum is 5 years. Over the years, observations get less and less.
...
$$\frac{1268}{2013+1100} = 0.4073241 = 40.7 \%$$

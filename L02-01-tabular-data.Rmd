...
Tabular data is the most common type of data. The ability to organise information systematically into rows and columns has been a cornerstone of data-driven decision-making for centuries. From handwritten ledgers to digital spreadsheets, tabular data has served as the foundation for understanding, managing, and extracting insights from a wide range of datasets.
...
### Types of Tabular Data {#types-of-tabular-data}
...
```{r tab-cross-section, warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
```{r tab-repeated-cross-section, warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
soep %>% 
  slice_head(n = 6) %>% 
  mutate(Id = seq(from=1, to=6, by=1)) %>% 
  mutate(Year = as.numeric(rep(format(Sys.Date(), "%Y"), 6))) %>% 
...
```{r tab-time-series, warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
```{r tab-panel-data, warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
soep %>% 
  slice_head(n = 6) %>% 
